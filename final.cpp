    ...
    */ 
    try {
        ...
        while (!globalConclusionQueue.empty()) {
            ...
            globalConclusionQueue.pop();  // Remove the front element from the queue
...
void searchCVL(string diagnosis) {
    ...
    for (const auto& rule : treatmentRuleList) {
        ...
        cout << "Checking (Treatment) rule: " << rule.number << endl;

        // If the rule's condition matches the diagnosis
        for (const auto& condition : rule.conditions) {
            if (condition.first == "diagnosis" && condition.second == diagnosis) {
                // Treatment is found for the given diagnosis
                cout << "Recommended Treatment for " << diagnosis << ": " << rule.conclusion.second << endl;

                // Add the treatment conclusion to derived variables
                derivedGlobalVariables["treatment"] = rule.conclusion.second;

                // You can further push any other variables into the global queue if needed
                // globalConclusionQueue.push(rule.conclusion.second); // In case we need to continue forward chaining
                return;
            }
        }
    }
    
    // If no matching treatment was found
